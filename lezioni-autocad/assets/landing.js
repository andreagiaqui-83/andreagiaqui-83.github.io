(() => {
  ...
  const sticky=$('.mobile-cta');let contactVisible=false,servicesVisible=false,dialogOpen=false,imageOpen=false;const visibleCTAs=new Set();
  const updateSticky=()=>{const hidden=contactVisible||servicesVisible||dialogOpen||imageOpen||visibleCTAs.size>0||!!document.activeElement?.closest('input,textarea,select');sticky.classList.toggle('is-hidden',hidden);sticky.inert=hidden;};
  ...
  if('IntersectionObserver'in window&&$('#servizi-progetto'))new IntersectionObserver(entries=>{servicesVisible=entries[0].isIntersecting;updateSticky();},{threshold:0}).observe($('#servizi-progetto'));
