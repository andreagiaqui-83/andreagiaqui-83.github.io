(() => {
  ...
  const sticky=$('.mobile-cta');let contactVisible=false,dialogOpen=false;
  const updateSticky=()=>{const hidden=contactVisible||dialogOpen||!!document.activeElement?.closest('input,textarea,select');sticky.classList.toggle('is-hidden',hidden);sticky.inert=hidden;};
