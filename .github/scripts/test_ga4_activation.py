"""Browser tests. All form writes and analytics collection are mocked, except
an optional single labelled live page_view smoke test after publication."""
import asyncio
import json
import mimetypes
import os
...
def params(request):
    result={k:v[-1] for k,v in parse_qs(urlparse(request.url).query).items()}
    data=request.post_data or ''
    if '\n' not in data:
        result.update({k:v[-1] for k,v in parse_qs(data).items()})
    return result
...
async def fixture(browser,width=1366,seed=None,smoke=False):
    ...
    async def router(route):
        ...
        # No form submissions ever reach the real backend.
        ...
        if (u.hostname or '').endswith('google-analytics.com') and '/collect' in u.path:
            p=params(req); state['google'].append('collect:'+p.get('en',''))
            state['events'].append(p)
            if smoke and p.get('en')=='page_view' and p.get('tid')==MID and state['allowedReal']==0:
                state['allowedReal']+=1
...
async def run(browser):
    ...
    try:
        ...
        await page.locator('[data-consent-open]').click();await page.locator('[data-choice="all"]').click()
        await page.wait_for_timeout(300)
        check('Repeated preference save does not duplicate page view',len([e for e in s['events'] if e.get('en')=='page_view'])==1)
        # The real contact API is intercepted above. No email/lead is sent.
        ...
        lead=[e for e in s['events'] if e.get('en')=='generate_lead']
        check('Mocked confirmed request generates one event',len(lead)==1)
        ...
        await page.locator('#ag-analytics').uncheck()
    ...
    # Existing preferences made while GA4 was inactive do not grant consent.
    ...
    if LIVE:
        ...
        try:
            ...
            report['realPageViewsSent']=1
            report['smoke']=s['actualResponses']
