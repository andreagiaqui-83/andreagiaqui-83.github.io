...
portrait_b64_path = Path('.github/scripts/portrait.b64')
html = html_path.read_text(encoding='utf-8')
js = js_path.read_text(encoding='utf-8')

# 1. Crea un vero file immagine della foto. Prima usa il payload separato già presente nel repository;
# come fallback usa photoData dal JS normalizzando padding e whitespace.
raw = portrait_b64_path.read_text(encoding='utf-8').strip() if portrait_b64_path.exists() else ''
if not raw:
    m = re.search(r"const\s+photoData\s*=\s*['\"]data:image/webp;base64,([^'\"]+)['\"]", js)
    if not m:
        raise SystemExit('photoData non trovata')
    raw = m.group(1)
raw = re.sub(r'\s+', '', raw)
raw += '=' * ((4 - len(raw) % 4) % 4)
...
# 2. Rimuove TUTTE le vecchie regole hero custom, poi inserisce una sola hero definitiva.
html = re.sub(r'/\* HERO-PROMO-V2 \*/.*?(?=</style>)', '', html, count=1, flags=re.S)
html = re.sub(r'/\* HERO AUTOCAD DEFINITIVA V13 \*/.*?(?=</style>)', '', html, count=1, flags=re.S)
final_css = r'''
/* HERO AUTOCAD DEFINITIVA V14 */
...
.hero-final-photo{position:relative;min-height:430px;border-radius:22px;overflow:hidden;border:1px solid rgba(105,229,255,.28);background:#06111c;box-shadow:0 20px 44px rgba(0,0,0,.32)}.hero-final-photo img{display:block;width:100%;height:100%;min-height:430px;object-fit:cover;object-position:center 12%;background:#06111c}.hero-final-photo-caption{position:absolute;left:12px;right:12px;bottom:12px;padding:12px 13px;border-radius:13px;background:rgba(5,18,31,.9);backdrop-filter:blur(7px)}.hero-final-photo-caption strong,.hero-final-photo-caption span{display:block}.hero-final-photo-caption span{margin-top:4px;color:#c7dae6;font-size:.8rem;line-height:1.35}
...
# 3. Hero unica e stabile.
hero = '''<section class="hero" id="top"><div class="container"><div class="hero-final" aria-label="Lezioni, corsi e ripetizioni AutoCAD 2D e 3D online"><div class="hero-final-photo"><img src="assets/andrea-giaquinto-autocad.webp?v=20260916-14" alt="Andrea Giaquinto, disegnatore AutoCAD certificato Autodesk ACU"><div class="hero-final-photo-caption"><strong>Andrea Giaquinto</strong><span>Disegnatore AutoCAD certificato Autodesk ACU · oltre 20 anni di esperienza CAD</span></div></div><div class="hero-final-copy"><span class="hero-final-kicker">LEZIONI · CORSI · RIPETIZIONI AUTOCAD</span><h1>Impara AutoCAD <span>davvero.</span></h1><p>Lezioni individuali e percorsi personalizzati per scuola, università e lavoro. Parti da zero, supera un blocco o perfeziona il metodo lavorando su esercizi, tavole e progetti reali.</p><div class="hero-final-chips"><span>Solo online</span><span>AutoCAD 2D + 3D</span><span>30 min gratis</span><span>15 €/ora</span><span>Maturità</span><span>Certificazioni Autodesk</span></div><div class="hero-final-actions"><a class="btn primary" href="#contatti">Prenota la prima lezione gratis →</a><a class="btn" href="#studenti">Trova il tuo percorso</a></div></div><div class="hero-final-cad"><img src="assets/autocad_dalla_bozza_alla_pianta.webp?v=20260916-14" alt="AutoCAD italiano 2027: dalle prime linee a una planimetria completa"><strong>Dalle basi ai tuoi progetti reali · impari facendo</strong></div></div><div class="hero-final-trust"><span>Certificato Autodesk ACU</span><span>Lezioni personalizzate 1:1</span><span>Corsi su misura</span><span>Online in tutta Italia</span></div></div><div class="container stats"><div><strong>2D + 3D</strong><small>Dal disegno tecnico alla modellazione</small></div><div><strong>Scuola, università, lavoro</strong><small>Lezioni costruite sui tuoi obiettivi</small></div><div><strong>15 €/ora</strong><small>Dopo la prima lezione gratuita di 30 minuti</small></div></div></section>'''
...
# 4. Rimuove le tre immagini decorative brutte senza toccare il testo della sezione.
html = re.sub(r'<div class="visual-strip" aria-label="Le fasi delle lezioni">.*?</div>\s*(?=</div></section>)', '', html, count=1, flags=re.S)

# 5. Ripristina SEMPRE la seconda immagine della roadmap, anche se il markup è stato alterato.
# Prima normalizza qualunque card 2 con roadmap-visual vuota.
html = re.sub(r'(<article class="roadmap-card">\s*)<div class="roadmap-visual">\s*</div>(\s*<h3>2\.)', r'\1<div class="roadmap-visual"><img src="assets/autocad_planimetria.webp?v=20260916-14" alt="AutoCAD italiano 2027: planimetria architettonica completa"></div>\2', html, count=1, flags=re.S)
# Poi aggiorna eventuale src già presente.
html = re.sub(r'src="assets/autocad_planimetria\.webp(?:\?v=[^"]*)?"', 'src="assets/autocad_planimetria.webp?v=20260916-14"', html, count=1)

# 6. Elimina loader foto e aggiorna build.
html = re.sub(r'<script id="heroPortraitLoader">.*?</script>', '', html, flags=re.S)
html = re.sub(r'data-build="[^"]+"', 'data-build="20260916-autocad-v14"', html, count=1)

checks = {
    'hero finale': 'class="hero-final"' in html,
    'foto fisica': 'andrea-giaquinto-autocad.webp?v=20260916-14' in html,
    ...
    'roadmap card 2 presente': 'autocad_planimetria.webp?v=20260916-14' in html,
...
print('PASS V14: hero stabile, foto fisica, immagini decorative rimosse, roadmap completa')
